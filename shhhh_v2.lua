...
oldNC = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
    ...
    if (method=="FindPartOnRayWithIgnoreList" or method=="FindPartOnRay") and State.SilentOn and not State.Panic then
        -- Only redirect rays that originate near the camera (gun rays)
        -- Roblox internal camera/movement rays have very different origins
        local ray = args[1]
        if ray and typeof(ray) == "Ray" then
            local originDist = (ray.Origin - Camera.CFrame.Position).Magnitude
            -- If the ray doesn't start near the camera it's not a gun ray, skip it
