                ...
                if char then
                    ...
                    if tp and hrp then
                        ...
                        if not State.FrameSkip then
                            -- Switch to Scriptable so we own the camera, then lerp smoothly
                            Camera.CameraType = Enum.CameraType.Scriptable
                            Camera.CFrame = Camera.CFrame:Lerp(CFrame.lookAt(Camera.CFrame.Position, pPos), sm)
                        end
                    end
                end
            else
                -- No target — give camera back to the game
                Camera.CameraType = Enum.CameraType.Custom
            end
        end
    else
        -- Aim assist off — always restore camera type
