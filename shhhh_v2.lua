...
-- ═══════════════════════════════════
-- SILENT AIM (Stellar-style: FireServer hook on MainEvent)
-- ═══════════════════════════════════
local MainEvent = ReplicatedStorage:FindFirstChild("MainEvent")
local UpdateMouseArg = "UpdateMousePos"  -- Da Hood default

local function GetSilentTarget(mousePos)
            ...
            if hrp then
                ...
                if vis then
                    ...
                    if d < cDist and (not Config.Checks.Wall or WallOK(Camera.CFrame.Position, hrp.Position)) then
                        cDist = d
                        closest = {Player=plr, HRP=hrp, Dist=dist3d, Air=IsInAir(plr)}
                    end
                end
            end
        end
...
-- Hook the game's own FireServer to intercept mouse position updates
if MainEvent then
    local oldFireServer
    oldFireServer = hookfunction(MainEvent.FireServer, newcclosure(function(self, arg, ...)
        if (arg == "UpdateMousePos" or arg == "GetMousePos" or arg == "MousePos") and State.SilentOn and not State.Panic then
            local mousePos = UserInputService:GetMouseLocation()
            local t = GetSilentTarget(mousePos)
            if t and math.random(1,300) <= Config.Silent.HitChance then
                local pred = AutoPred()
                local hrp = t.HRP
                -- Apply camera-relative prediction like Stellar does
                local vel = hrp.AssemblyLinearVelocity
                local relVel = Camera.CFrame:VectorToObjectSpace(vel)
                local predVel = relVel * pred * Vector3.new(1,0,1)
                local predPos = hrp.Position + Camera.CFrame:VectorToWorldSpace(predVel)
                if Config.Shake.Enabled then
                    predPos = predPos + Vector3.new(
                        math.random(-Config.Shake.X,Config.Shake.X)*.1,
                        math.random(-Config.Shake.Y,Config.Shake.Y)*.1,
                        math.random(-Config.Shake.Z,Config.Shake.Z)*.1
                    )
                end
                return oldFireServer(self, arg, predPos, ...)
            end
        end
...
RunService.RenderStepped:Connect(function()
    ...
    -- FOV Circles (follow mouse cursor)
    local mousePos = UserInputService:GetMouseLocation()
    if SilentFov then
        SilentFov.Position  = mousePos + Vector2.new(Config.Fov.Silent.Set[1], Config.Fov.Silent.Set[2])
        SilentFov.Radius    = Config.Fov.Silent.Size
        SilentFov.Visible   = Config.Fov.Silent.Visible and State.SilentOn and not State.Panic
    end
    if AimFov then
        AimFov.Position  = mousePos + Vector2.new(Config.Fov.AimAssist.Set[1], Config.Fov.AimAssist.Set[2])
        AimFov.Radius    = Config.Fov.AimAssist.Size
