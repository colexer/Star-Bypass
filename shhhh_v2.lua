...
oldNC = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
    ...
    if (method=="FindPartOnRayWithIgnoreList" or method=="FindPartOnRay") and State.SilentOn and not State.Panic then
        -- Only redirect rays that originate near the camera AND are long (gun rays)
        -- Camera occlusion/zoom rays: short (<100 studs), start near camera  
        -- Gun rays: long (~1000 studs), start near camera
        -- We want ONLY gun rays, so filter by length >= 500
        local ray = args[1]
        if ray and typeof(ray) == "Ray" then
            local originDist = (ray.Origin - Camera.CFrame.Position).Magnitude
            local rayLen     = ray.Direction.Magnitude
            if originDist > 15 or rayLen < 500 then
